...
namespace RecordsAttachment
{
    public partial class Frm_Main : Form
    {
        ...
        private void Btn_Start_Click(object sender, EventArgs e)
        {
            ...
            bool SelectKF = false;
            #region Version
            var version = Path.GetFullPath(System.IO.Directory.GetCurrentDirectory() + @"/selenium-manager/macos/Selenium-macos.txt");
            if (File.Exists(version) || DateTime.Now.Date > new DateTime(2023, 02, 10))
            {
                var line = "";
                if (!File.Exists(version))
                {
                    using (var txtFile = File.AppendText(version))
                    {
                        txtFile.WriteLine("selenium 4.6.0");
                    }
                   
                }
                try
                {
                    line = File.ReadAllText(version);
                }
                catch{}
                if (line.Contains("selenium 4.6.0"))
                {
            ...
            if (!SelectKF)
            {
                LbL_Message.Text = "Select List Type!";
                LbL_Message.Visible = true;
                Btn_Start.Enabled = true;
                return;
            }
            ...
            if (Str_Ptype== "Hag")
            {
                Lst_data = Lst_data.OrderBy(l => l.Split('*')[0]).OrderBy(l => l.Split('*')[1]).ToList();
                try
                {
                    File.WriteAllLines(SourceFile, Lst_data);
                }
                catch { }
        ...
        private void Hag_DoJob(string data)
        {
            ...
            try
            {
                ...
                while (stepcounter < 8)
                {
                    ...
                    string str_el = "";
                    if (Str_KFtype == "Kh")
                        str_el = "CPC_urvRemainViewSolr105_ddlFilterAttachParam1";
                    else
                        str_el = "CPC_urvRemainViewSolr103_ddlFilterAttachParam1";

                    if (step02 == 0 && FindElement(str_el, FindBy.ById))
                    {
                        IJavaScriptExecutor js3 = (IJavaScriptExecutor)driver;
                        string script3 = "var ddl = document.getElementById('"+str_el+"');  ddl.selectedIndex = 1 ";
                    ...
                    //Thread.Sleep(3000);
                    if (Str_KFtype == "Kh")
                        str_el = "CPC_urvRemainViewSolr105_ForoushandeNationalCode";
                    else
                        str_el = "CPC_urvRemainViewSolr103_KharidarNationalCode";
                    
                    if (step2 == 0 && step02 == 1 && FindElement(str_el, FindBy.ById))
                    {
                        IJavaScriptExecutor js3 = (IJavaScriptExecutor)driver;
                        string script3 = "var txt = document.getElementById('" + str_el + "');  txt.value =" + data.Split('*')[0];
                        js3.ExecuteScript(script3);
                        step2 = 1;
                        ++stepcounter;
                    }
                    ...
                    if (Str_KFtype == "Kh")
                        str_el = "CPC_urvRemainViewSolr105_ListPage1_ddlItemPerPage";
                    else
                        str_el = "CPC_urvRemainViewSolr103_ListPage1_ddlItemPerPage";
                    
                    if (step3 == 0 && step2 == 1 && FindElement(str_el, FindBy.ById))
                    {
                        //wait.Until(w => w.FindElements(By.Id("CPC_urvRemainViewSolr103_ListPage1_ddlItemPerPage")));
                        IWebElement element = driver.FindElement(By.Id(str_el));
                ...
                while (stepcounter < 5)
                {
                    ...
                    if (step9 == 0 && FindElement("CPC_urvRTCMIS_NationalCode", FindBy.ById))
                    {
                        ...
                        script9 += "var txt = document.getElementById('CPC_urvRTCMIS_NationalCode');txt.value = '" + data.Split('*')[0] + "';";
        ...
        private void Hog_DoJob(string data)
        {
            ...
            try
            {
                ...
                while (stepcounter < 8)
                {
                    ...
                    {
                        if (Str_KFtype == "Kh")
                            str_el = "CPC_urvRemainViewSolr105_ForoushandeEconomicNO";
                        else
                            str_el = "CPC_urvRemainViewSolr103_KharidarEconomicNO";
                    ...
                    else
                    {
                        ...
                        {
                            IJavaScriptExecutor js3 = (IJavaScriptExecutor)driver;
                            string script3 = "var txt = document.getElementById('"+ str_el + "');  txt.value =" +  data;
                            js3.ExecuteScript(script3);
                            step2 = 1;
                            ++stepcounter;
                        }
                    ...
                    {
                        IJavaScriptExecutor js8 = (IJavaScriptExecutor)driver;
                        string script8 = "  var div2 = document.getElementsByClassName('bootbox-body')[0];var a_= div2.querySelectorAll('a')[1]; a_.click();  ";
                ...
                while (stepcounter < 5)
                {
                    ...
                    if (step9 == 0 && FindElement("CPC_urvRTCITMS_NationalCode", FindBy.ById))
                    {
                        ...
                        IJavaScriptExecutor js9 = (IJavaScriptExecutor)driver;
                        string script9 = " var div = document.getElementsByClassName('navbar-form dropdown')[3];div.className = 'navbar-form dropdown open';";
                        script9 += "var txt = document.getElementById('CPC_urvRTCITMS_NationalCode');txt.value = '" + (data.Contains('*') ? data.Split('*')[1] : data)  + "';";
                        script9 += "var btn = div.querySelectorAll('button')[1];";
                        script9 += "btn.click(); ";
                        js9.ExecuteScript(script9);
                        step9 = 1;
                    ...
                    if (rows_count > 2)
                    {
                        IJavaScriptExecutor js10_ = (IJavaScriptExecutor)driver;
                        string script10_ = " var div = document.getElementsByClassName('navbar-form dropdown')[4];div.className = 'navbar-form dropdown open';";
                        script10_ += "var txt = document.getElementById('CPC_urvRTCITMS_Com_TIN');txt.value = '" + data.Split('*')[0] + "';";
                        script10_ += "var btn = div.querySelectorAll('button')[1];";
                        script10_ += "btn.click(); ";
                        js10_.ExecuteScript(script10_);
